...
На рисунке 3 <<работа в цифровом режиме>> показан источник питания, резистор, который ограничивает ток и определяет яркость светодиода. Светодиод подключен к питанию, а микроконтроллер подключают
...
Когда порт настроен как цифровой вход его сопротивление бесконечно, контакт никуда не подключен ни к земле ни к питанию, поэтому ток никуда не течет. Любое напряжение на
...
Пример настройки задежки:

[source,cpp]
----
void SetTimeout(std::uint32_t delayInms)
{
  TIM2::PSC::Set(timPresc - 1U); частоты
  TIM2::ARR::Write(delayInms * timPeriod); 
  TIM2::SR::UIF::Set(0); 
  TIM2::CNT::Set(0); 
  TIM2::CR1::CEN::Set(1); 
}
