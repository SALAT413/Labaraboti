...
static void Create() {
if (s_handle == nullptr) {
s_handle = xQueueCreate(LED_MAILBOX_SIZE, sizeof(LedMailBoxDataType));
}
}

static bool Send(const LedMailBoxDataType& value, TickType_t timeout = 0) {
if (s_handle == nullptr) return false;
return xQueueSend(s_handle, &value, timeout) == pdPASS;
}

static bool Receive(LedMailBoxDataType& value, TickType_t timeout = portMAX_DELAY) {
if (s_handle == nullptr) return false;
return xQueueReceive(s_handle, &value, timeout) == pdPASS;
}
...
static void Create() {
if (s_handle == nullptr) {
s_handle = xQueueCreate(UART_MAILBOX_SIZE, sizeof(UartMailBoxDataType));
}
}

static bool Send(const UartMailBoxDataType& value, TickType_t timeout = 0) {
if (s_handle == nullptr) return false;
return xQueueSend(s_handle, &value, timeout) == pdPASS;
}

static bool Receive(UartMailBoxDataType& value, TickType_t timeout = pdMS_TO_TICKS(100)) {
if (s_handle == nullptr) return false;
return xQueueReceive(s_handle, &value, timeout) == pdPASS;
}
...
LedMailBox::Send(m_lastFilteredVoltage);
UartMailBox::Send(m_lastFilteredVoltage);
}
}
...
for (;;) {
if (LedMailBox::Receive(voltage, portMAX_DELAY)) {
UpdateLEDs(voltage);
}
vTaskDelay(pdMS_TO_TICKS(10));
}
...
for (;;) {
if (UartMailBox::Receive(voltage, pdMS_TO_TICKS(100))) {
